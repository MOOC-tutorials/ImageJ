...
/** This class handles drag&drop onto JFileChoosers. */ 
 public class DragAndDropHandler extends TransferHandler {
	...
	public boolean importData(JComponent comp, Transferable t) {
		...
		for (DataFlavor dataFlavor : transferFlavors) {
			try {
				...
				fileList = setFileList(t, dataFlavor, fileList);
				if (fileList == null || fileList.isEmpty()) continue;  //this data flavor did not work

				File firstFile = fileList.get(0);
				getSelectedFiles(fileList, firstFile);
	...
	private void getSelectedFiles(java.util.List<File> fileList, File firstFile) {
	...
	private java.util.List<File> setFileList(Transferable t, DataFlavor dataFlavor, java.util.List<File> fileList)
		...
		if (dataFlavor.isFlavorJavaFileListType()) {
			...
			fileList = new ArrayList<>(strs.length);
			for (String s : strs) {
				if (s.length() > 0) {
